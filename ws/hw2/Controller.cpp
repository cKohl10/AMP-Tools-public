...
Controller::Controller(amp::Problem2D p, bool rightTurning){
    ...
    lastFollowObstacle = -2; //-2 means none
}
    ...
    */
    ...
    for (int j = 0; j < collisionDexs.size(); j++){
        if ((collisionDexs[j] != targetObstacleIndex) && (collisionDexs[j] > -1)){
            ...
            //if (collisionDexs[j] == lastFollowObstacle){
            //    std::cout << "Collision with same object" << std::endl;
            //    continue;
            //}

            //Set new target obstacle index
            lastFollowObstacle = targetObstacleIndex;
    ...
    if (collisionCount > 1){
        std::cout << "ERROR: More than one collision detected" << std::endl;
    }
...
void Controller::clearLastFollowObstacle(){
    lastFollowObstacle = -2;
    return;
}